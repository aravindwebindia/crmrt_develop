...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Get sale orders where is_invoiced = 1 and proforma invoice not yet generated
        $query = "SELECT id, bc_id, sub_total, grand_total, is_invoiced 
                  FROM sale_orders 
                  WHERE is_invoiced = 1 
                  AND (is_proforma_generated = 0 OR is_proforma_generated IS NULL)";
        ...
        foreach ($sale_orders as $sale_order) {
            try {
                // Start transaction for each sale order
                ...
                $proforma_invoice_no = 'PI-' . str_pad($count + 1, 4, '0', STR_PAD_LEFT);
                ...
                $sale_order_details_stmt->execute([$sale_order['id']]);
                ...
                $update_sale_order_stmt->execute([$sale_order['id']]);
                ...
                $pdo->rollBack();
                $errors[] = "Failed to generate proforma invoice for sale order {$sale_order['id']}: " . $e->getMessage();
                }
        }
        
        echo json_encode([
            'success' => true,
            'message' => "Processed {$generated_count} sale orders",
            'generated_count' => $generated_count,
            'total_found' => count($sale_orders),
            'errors' => $errors
        ]);
